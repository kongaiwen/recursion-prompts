...
var augmentElements = function(array, aug) {
  ...
  arr.push(array[0]);
  array.shift();
  return arr.concat(augmentElements(array, aug));
};
...
var minimizeZeroes = function(array) {
  ...
  if (array[0] === 0 && array[1] === 0) {
    ...
    arr.push(array[0]);
    array.shift();
    return arr.concat(minimizeZeroes(array));
  }
};
...
var numToText = function(str) {
  ...
  if (words.length === 1) {
    if (isNaN(parseInt(words[0])) === false) {
      return sentence += numbers[words[0]];
    } else {
      return sentence += words[0];
    }
  } else {
    if (isNaN(parseInt(words[0])) === false) {
      sentence += numbers[words[0]] + ' ';
      words.shift();
      str = words.join(' ');
      return sentence + numToText(str);
    } else {
      sentence += words[0] + ' ';
      words.shift();
      str = words.join(' ');
...
var tagCount = function(tag, node) {
  ...
  for (var i = 0; i < node.children.length; i++) {
    ...
    counter += tagCount(tag, node.children[i]);
  }
...
var binarySearch = function(array, target, min, max) {
  ...
  var median = Math.floor((max + min) / 2);
...
var mergeSort = function(array) {
  ...
  var median = Math.ceil(array.length / 2);

  var firstHalf = mergeSort(array.slice(0, median));
  var secondHalf = mergeSort(array.slice(median, array.length))
  ...
  if (i !== firstHalf.length) {
    sortedArray = sortedArray.concat(firstHalf.slice(i));
  }

  if (j !== secondHalf.length) {
    sortedArray = sortedArray.concat(secondHalf.slice(j));
  }
